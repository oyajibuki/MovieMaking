...
| 声色変換 | 身バレ防止用。「女性の声」「太い男の声」「子供の声」などのプリセットから選ぶだけ。ピッチとフォルマントを同時に動かす |
...
区間演算・字幕リマップ・SRT/ASS 出力・メディア種別判定・声色プリセットは
重い依存なしで実行できる（48 件）。
